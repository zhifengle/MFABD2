...
`assets/resource/bridge/pipeline/Arbitrage.json` 已移除，跑商购买回落到 base/pc 层的默认 Arbitrage 链（`Arbitrage_BuyItem → Arbitrage_Buy_ToSelect / Arbitrage_Buy_SkipSelect`）。收藏扫描频率由 `购买收藏扫描频率` 选项控制：`每周` 在周内仅扫描一次，已有周标记时跳过；`每次` 强制扫描；`从不` 跳过选星并直接购买。商店动作与一键购买由 base/pc 层 `Arbitrage_Action_Hub` 处理，避免 `Arbitrage_BuyItem` 达到 `max_hit` 后经默认空节点返回、再次进入购买门卫形成死循环。
...
**现象：** 卡带进入箱庭后，`Collect_FindTeleporCircle0.9` 显示画面已改变并成功返回，但没有进入“移动魔法阵”区域地图；后续 `Collect_OperationsMain_Sandplay` 失败、复位箱庭并重复同一路径。实机可能观察到脚本触发 NPC 对话。

**根因与事实边界：** 本质是角色的当前位置距 NPC 过近。PC 的 `Collect_TeleporCircle_Ico_Enter` 识别右下角通用 `F` 交互按钮；该按钮在传送阵旁可用于打开区域地图，但在 NPC 旁代表对话。`SmartAction` 只根据动作前后画面差判断操作是否生效；NPC 对话同样会造成明显画面变化，因此会被误当成“已打开区域地图”。问题发生在 Bridge 按数字键 `1` 选传送阵之前，与该按键路由无关。

**当前处理：** 暂不修改 pipeline。运行前应确保角色不在 NPC 交互范围内；若日志反复出现“`Collect_FindTeleporCircle0.9` 成功 → `Collect_OperationsMain_Sandplay` 失败 → 复位箱庭”，或画面进入 NPC 对话，立即停止任务，手动离开 NPC 后再运行。后续若修复，不能只调整帧差阈值；需要为通用 `F` 交互增加传送阵语义门卫，或建立可靠的独立地图入口。

**实机键位记录：** 箱庭中按主键盘 `M`（Windows VK `77`）会打开当前地区的本地地图。在本地地图页再次按 `M` 不会关闭该页；按 `Esc`（Windows VK `27`）可关闭并返回箱庭。
...
按键传送成功后必须进入 `Collect_Skill_Entry`，由它注册 `QuickSkill_Egress → Collect_Skill_Egress` 再调用技能模块。Bridge 覆盖不得直接进入 `Collect_Skill_Start`；否则技能完成后没有返回导航主线的出口锚，会超时退回快速卡带菜单，并经 `Collect_PackEnd_Ck` 对尚未采集后续地图的卡带提前写入完成标记。
