...
**现象：** 第四图之后命中“没有已激活的魔法阵”并继续右翻页，能够识别“通往暗夜世界入口之路”，但随后 `Collect_TargetMap4_Special` 失败并离开剧情类别；外层任务报告完成，`Story_19_被遗忘的战争` 没有完成记录。

**根因：** 特殊页按设计启动新的 `Collect_TargetMap3` 以复用第三图之后的导航状态，但历史修复只改了后继，没有把当前特殊地图名加入 `Collect_TargetMap3` 白名单。新节点因此在进入传送阵、无阵和翻页候选链之前就识别失败并走默认 `Global_Null`。

**当前实现：** base 与 PC 的 `Collect_TargetMap3` 白名单均包含“通往暗夜世界入口之路”，PC 同时兼容“通往夜世界入口的路”OCR 变体；`Collect_TargetMap4_Special` 继续使用普通后继启动新的第三图状态。它不点击地图、不绕过无阵 OCR，也不直接伪造完成标记；最终仍必须经 `Collect_QuickCart_FastUI → Collect_PackEnd_Ck` 才能写入 Story19 完成记录。

**验证方式：** 离线回归检查最终 `base → pc → bridge` 节点以普通后继进入 `Collect_TargetMap3`，且第三图最终白名单包含两种特殊地图名。实机验收必须看到 Story19 特殊图后成功启动 `Collect_TargetMap3`、继续原导航链，并最终输出 `Story_19_被遗忘的战争` 完成标记。
