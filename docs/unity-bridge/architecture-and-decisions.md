...
`UnityBridgeClient.click` 对 `no-click-handler` 做有限重试（`NO_CLICK_HANDLER_RETRIES` 次，间隔 `NO_CLICK_HANDLER_RETRY_DELAY_S`）。这是因为 Unity UI 的 `IPointerClickHandler` 注册可能晚于视觉渲染——OCR/ColorMatch 通过时按钮文字和颜色已就绪，但交互组件尚未挂载，导致 RaycastAll 命中对象但无 handler。重试只给交互组件更多初始化时间，不改变最终失败语义：重试耗尽后仍返回 False。这不是"全局吞掉"（已否决方案 #9），因为其他失败类型（超时、插件错误等）不重试，且 `no-click-handler` 仍会如实记录和返回。
